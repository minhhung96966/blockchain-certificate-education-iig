...
function startBroadcastCerts() {
    ...
    if (items.length == 0) {
        ...
        $.post("/issuer/broadcastCertificate", postData, function(result) {
            ...
            if (result.success == 1) {
                swal({
                    title : "Good job!",
                    text : "Your certificate has been broadcasted successfully.",
                    type : "success",
                    showCancelButton : false,
                    confirmButtonText : "OK",
                    closeOnConfirm : false
                }, function() {
                    window.location.href = "broadcasted_certificate_list";
                });
...
var TableInit = function() {
    ...
    oTableInit.Init = function() {
        $('#table-list').bootstrapTable({
            url : '/issuer/postCertsInfoList', // 请求后台的URL（*）
            method : 'post', // 请求方式（*）
    ...
    oTableInit.queryParams = function(params) {
        var queryConditionJson = {
            ...
            'queryObject' : '{"cstate":"merged","id":"matchAll_'+params.searchText+'"}',
            'rowCount' : 1,
...
function displayCertificate(id) {
    id = $.base64('encode', id);
    $.getJSON("/issuer/getCertsInfo/" + id, function(result) {
