...
function showDetails(){
    ...
    if (items.length == 0) {
        ...
        $("#certsInfo").JSONView({})
        $("#certsInfo").JSONView(items[0], { collapsed: true });
    }
}
...
function generateQr() {
	...
	var items= $('#table-list').bootstrapTable('getSelections');
    if(items.length == 1){
    	var downJson = JSON.stringify(items[0]);
...
var TableInit = function () {
    ...
    oTableInit.queryParams = function (params) {
    	...
    	var queryObject = '{"cstate":"revoked","recipient.email":"'+studentName+'","id":"matchAll_'+params.searchText+'"}'
        var queryConditionJson = {'pageNumber':params.pageNumber ,'pageSize':params.pageSize ,'queryObject':queryObject,'rowCount':1,'sortColumn':'apply_time','sortType':'DESC'};
