...
/**
 * 
 * @Title: CHexConver.java
 * @Package com.minhhung.utils
 * @Description: TODO String/Byte
 * @author lijian@dzs.mobi
 * @version V1.0
 */
public class CHexConver {
	 ...
	 */
	public static String byte2HexStr(byte[] newData, int iLen) {
		StringBuilder sb = new StringBuilder();
		for (int n = 0; n < iLen; n++) {
			sb.append(mChars[(newData[n] & 0xFF) >> 4]);
			sb.append(mChars[newData[n] & 0x0F]);
			sb.append(' ');
		}
	...
	/**
	 * 将一个长整数转换位字节数组(8个字节)，b[0]存储高位字符，小端
	 * 
	 * @param l
	 *            长整数
	 * @return 代表长整数的字节数组
